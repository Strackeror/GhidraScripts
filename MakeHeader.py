...
try:
    from ghidra.ghidra_builtins import currentProgram, askString, askFile
except:
    pass

basename = askString("namespace", "choose namespace to export", None)
symbolTable = currentProgram.getSymbolTable()
functionManager = currentProgram.getFunctionManager()
main_namespace = symbolTable.getNamespace(basename, None)  # type: ignore
indent = ""
...
def handleSymbol(symbol):
    print(
        indent,
        "static void *{0} = (void*)0x{1};".format(
            escapeName(symbol.getName()), symbol.getAddress()
        ),
        file=targetFile,
        sep="",
    )
...
def handleNamespace(namespace):
    global indent
    print(
        indent, "namespace {0} {{".format(namespace.getName()), file=targetFile, sep=""
    )
    indent = indent + "  "
